...
on-demand fault injections (7 modes) with immediate ML evaluation & WebSocket broadcasting.
"""
...
async def inject_fault_endpoint(req: FaultInjectRequest):
    ...
    Immediately passes the altered reading through Tier 1 Anomaly Detection & SHAP Explainer,
    records the anomaly entry, and broadcasts ANOMALY_DETECTED over live WebSocket.
    """
    ...
    if altered_reading:
        ...
        # Ensure injected reading is explicitly tagged if model threshold is close
        ...
        imputed = imputer.suggest_correction(prim_feat, bad_v, spatial_neighbors=neighbors)
