...
Includes ALL 14 stations/places across both OpenML Goa Dataset 43409 and Secondary Local Indian Climate Dataset (2024-2025).
Supports interactive controls (Start, Stop, Reset, Speed multipliers) and 7 on-demand fault injection modes.
"""
...
class VirtualAWSSimulator:
    ...
    def generate_reading(self, station_id: str, current_time: Optional[datetime.datetime] = None) -> Optional[Dict[str, Any]]:
        ...
        rainfall = round(max(0.0, np.random.exponential(1.5) if random.random() > 0.85 else 0.0), 2)
        ...
        if injection and injection["remaining_steps"] > 0:
            ...
            elif f_type in ["multivariate_fault"]:
