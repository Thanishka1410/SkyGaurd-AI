...
import { Droplets, Thermometer, Wind, CheckCircle2 } from 'lucide-react';
...
export const DisasterRiskPage: React.FC<DisasterRiskPageProps> = ({ risks }) => {
  const hazards = risks?.hazards || {
    flood: {
      ...
      risk_level: 'LOW',
      confidence: 0.88,
      contributing_factors: ['Precipitation: 0.0 mm/hr', 'Relative Humidity: 78.0%', 'Pressure: 1012.0 hPa']
    },
    heatwave: {
      risk_score: 22.0,
      risk_level: 'LOW',
      confidence: 0.92,
      contributing_factors: ['Ambient Temperature: 28.5 °C', 'Humidity Index: 78.0%']
    },
    cyclone: {
      risk_score: 14.2,
      risk_level: 'LOW',
      confidence: 0.85,
      contributing_factors: ['Pressure: 1012.0 hPa', 'Wind Speed: 12.0 km/h']
    }
  };
  ...
  return (
    <div className="space-y-8 pb-12 font-sans text-slate-900">
      ...
      {/* Banner */}
      <div className="luxury-card p-8 bg-gradient-to-r from-emerald-500/10 via-emerald-500/5 to-white border-l-8 border-l-emerald-500 border border-emerald-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center space-x-3">
            ...
            <span className="text-xs text-emerald-700 font-mono font-semibold">(Value-Add Risk Intelligence Layer)</span>
          </div>
        ...
        <div className="text-left md:text-right bg-white px-6 py-4 rounded-xl border border-slate-200 shadow-sm font-mono">
          <div className="text-xs text-slate-500 font-bold">COMPOSITE RISK STATUS</div>
          <div className="text-3xl font-black text-emerald-600 mt-1">{risks?.composite_risk_level || 'LOW'}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="luxury-card p-6 bg-white border border-slate-200 border-t-4 border-t-blue-500 shadow-sm space-y-4 font-sans">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              <div className="p-2 rounded-xl bg-blue-50 border border-blue-200 text-blue-600">
                <Droplets className="w-5 h-5 text-blue-600" />
              </div>
              <h3 className="font-extrabold text-base text-slate-900 font-display">Flood & Rainfall</h3>
            </div>
          ...
          <div className="space-y-1.5 font-mono">
            <div className="flex items-center justify-between text-xs font-semibold text-slate-600">
              ...
              <span className="text-blue-600 font-extrabold text-sm">{hazards.flood.risk_score} / 100</span>
            </div>
            <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden border border-slate-200">
              <div className="bg-blue-600 h-full" style={{ width: `${hazards.flood.risk_score}%` }}></div>
            </div>
          </div>

          <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 text-xs space-y-2 font-mono">
            <span className="font-bold text-slate-500 block uppercase text-[10px]">Contributing Environmental Drivers:</span>
            {hazards.flood.contributing_factors.map((f, i) => (
              <div key={i} className="text-slate-800 text-xs flex items-center space-x-2">
        ...
        <div className="luxury-card p-6 bg-white border border-slate-200 border-t-4 border-t-amber-500 shadow-sm space-y-4 font-sans">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              ...
              <h3 className="font-extrabold text-base text-slate-900 font-display">Thermal Stress</h3>
            </div>
          ...
          <div className="space-y-1.5 font-mono">
            <div className="flex items-center justify-between text-xs font-semibold text-slate-600">
              <span>RISK INDEX SCORE</span>
              <span className="text-amber-600 font-extrabold text-sm">{hazards.heatwave.risk_score} / 100</span>
            </div>
            <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden border border-slate-200">
              <div className="bg-amber-500 h-full" style={{ width: `${hazards.heatwave.risk_score}%` }}></div>
            </div>
          </div>
        ...
        <div className="luxury-card p-6 bg-white border border-slate-200 border-t-4 border-t-emerald-500 shadow-sm space-y-4 font-sans">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              <div className="p-2 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-600">
                <Wind className="w-5 h-5 text-emerald-600" />
              </div>
              <h3 className="font-extrabold text-base text-slate-900 font-display">Cyclone & Storm</h3>
            </div>
          ...
          <div className="space-y-1.5 font-mono">
            <div className="flex items-center justify-between text-xs font-semibold text-slate-600">
              <span>RISK INDEX SCORE</span>
              <span className="text-emerald-600 font-extrabold text-sm">{hazards.cyclone.risk_score} / 100</span>
            </div>
            <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden border border-slate-200">
              <div className="bg-emerald-500 h-full" style={{ width: `${hazards.cyclone.risk_score}%` }}></div>
            </div>
          </div>
      ...
      <div className="luxury-card p-5 bg-white border border-slate-200 text-slate-700 text-xs font-mono flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
        ...
        <span className="font-bold text-blue-600">TAG: WEATHER_API_SYNCHRONIZED</span>
      </div>
