...
export const SpatialConsensusPanel: React.FC<SpatialConsensusPanelProps> = ({ spatialVerdict = 'NOT_CHECKED' }) => {
  const isComm = spatialVerdict.includes('COMMUNICATION') || spatialVerdict.includes('N/A');
  const isGenuineEvent = !isComm && (spatialVerdict.includes('GENUINE') || spatialVerdict.includes('CORROBORATED'));
  const isIsolatedFault = !isComm && (spatialVerdict.includes('CONTRADICTED') || spatialVerdict.includes('ISOLATED'));
  ...
  return (
    <div className="luxury-card p-5 sm:p-6 bg-white border border-slate-200 shadow-sm space-y-4 font-sans min-w-0 w-full overflow-hidden">
      ...
      <div className="p-4 bg-slate-50/80 border border-slate-200 rounded-xl flex items-center space-x-3 text-xs font-mono">
        {isComm ? (
          <>
            <AlertTriangle className="w-5 h-5 text-purple-600 shrink-0" />
            <span className="text-purple-800 leading-relaxed">
              VERDICT: Deterministic Communication Failure. Spatial consensus check bypassed (station offline or stream packet error).
            </span>
          </>
        ) : isGenuineEvent ? (
          <>
