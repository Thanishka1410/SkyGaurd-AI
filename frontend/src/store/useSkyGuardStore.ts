    ...
    try {
      ...
      wsSocket.onmessage = (event) => {
        try {
          ...
          } else if (payload.event === 'ANOMALIES_RESET') {
            set({
              anomalies: [],
              disasterRisks: payload.disaster_risks ?? get().disasterRisks,
              liveReadings: payload.readings ?? get().liveReadings
            ...
            readings.forEach(r => {
              ...
              if (isAnom || hasFault) {
                ...
                const streamAnom = {
                  ...
                  rootCause: rootCause,
                  confidence: evalData.confidence || 0.95,
                  isolation_forest_score: evalData.isolation_forest_score || -0.25,
                  spatial_verdict: evalData.spatial_verdict || 'CONTRADICTED_BY_NEIGHBORS (ISOLATED SENSOR FAULT)',
                  contributing_factors: factors,
                  imputed_value_suggestion: r.imputed_suggestion
                };
  ...
  injectFault: async (stationId: string, faultType: string, param: string, mag: number) => {
    try {
      ...
      if (res.ok) {
        ...
        if (data.anomaly) {
          const currentAnomalies = get().anomalies;
          const exists = currentAnomalies.some(a => a.id === data.anomaly.id);
          if (!exists) {
            set({ anomalies: [data.anomaly, ...currentAnomalies].slice(0, 100) });
          }
        }
      }
      get().fetchInitialData();
...
export function evaluateTier1Anomaly(
  ...
  if (backendEval && typeof backendEval.isolation_forest_score === 'number') {
    const backendRaw = Math.abs(backendEval.isolation_forest_score);
    if (backendEval.is_anomaly) {
      rawScore = Math.max(rawScore, Math.min(1.0, Math.max(0.68, backendRaw)));
    } else {
      rawScore = Math.max(rawScore, Math.min(0.34, backendRaw));
    }
  }
  ...
  const isTempFlagged = absTempDev > 6.0 || temperature > 40.0 || (statusBadge !== 'NORMAL' && tempIndex >= Math.max(pressIndex, humIndex));
  const isPressFlagged = absPressDev > 10.0 || pressure < 995.0 || (statusBadge !== 'NORMAL' && pressIndex >= Math.max(tempIndex, humIndex));
  const isHumFlagged = absHumDev > 22.0 || humidity > 95.0 || humidity < 25.0 || (statusBadge !== 'NORMAL' && humIndex >= Math.max(tempIndex, pressIndex));

  let confidencePct = 95;
  if (statusBadge === 'CRITICAL ANOMALY') {
    confidencePct = Math.round(86 + Math.min(13, anomalyScore * 14));
  } else if (statusBadge === 'WARNING / DRIFT') {
    confidencePct = Math.round(76 + Math.min(12, anomalyScore * 14));
  } else {
    confidencePct = Math.round(92 + Math.min(7, (1 - anomalyScore) * 8));
  }

  let rootCause = 'nominal';
  let reasonText = 'All sensor readings within expected operational range.';

  if (statusBadge === 'CRITICAL ANOMALY') {
    if (injectedFaultType && injectedFaultType !== 'NONE') {
      rootCause = injectedFaultType;
      reasonText = `Live fault injection detected: ${injectedFaultType.replace(/_/g, ' ').toUpperCase()}`;
    } else if (isTempFlagged) {
      rootCause = 'temperature_spike';
      reasonText = `Temperature anomaly: ${tempDelta > 0 ? '+' : ''}${tempDelta.toFixed(1)}°C delta from baseline (${temperature.toFixed(1)}°C).`;
    } else if (isPressFlagged) {
      rootCause = 'pressure_drop';
      reasonText = `Barometric anomaly: ${pressDelta.toFixed(1)} hPa pressure drop detected (${pressure.toFixed(1)} hPa).`;
    } else if (isHumFlagged) {
      rootCause = 'humidity_extremum';
      reasonText = `Relative humidity anomaly: ${humDelta > 0 ? '+' : ''}${humDelta.toFixed(1)}% shift (${humidity.toFixed(1)}%).`;
    } else {
      rootCause = 'multivariate_isolation_forest';
      reasonText = `IsolationForest detected multivariate anomaly score ${anomalyScore.toFixed(2)}.`;
    }
  } else if (statusBadge === 'WARNING / DRIFT') {
    rootCause = 'sensor_drift';
    reasonText = `Sensor parameter drift detected. Score: ${anomalyScore.toFixed(2)}. Monitor closely for degradation.`;
  }

  const shapT = Number((tempDelta * 0.042).toFixed(4));
  const shapP = Number((pressDelta * 0.038).toFixed(4));
  const shapRH = Number((humDelta * 0.018).toFixed(4));
...
export function normalizeAnomalyRecord(raw: any): AnomalyRecord {
  ...
  return {
    ...
    isolation_forest_score: raw.isolation_forest_score ?? raw.isolationForestScore ?? -0.25,
    spatial_verdict: raw.spatial_verdict || raw.spatialVerdict || 'CONTRADICTED_BY_NEIGHBORS (ISOLATED SENSOR FAULT)',
...
export function addOrUpdateAnomalyRecord(
  ...
  return [incoming, ...anomalies].slice(0, 50);
}
